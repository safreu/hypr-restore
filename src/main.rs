...
fn main() -> std::io::Result<()> {
    ...
    let ignored_handler = IgnoredClasses::new("ignore.txt");
    ...
    for line in instance.reader().lines() {
        match line {
            Ok(line) => {
                ...
                if line.contains("openwindow") && !ignored_handler.should_ignore(&parts[3]) {
                    table.insert(
                        EventEntry::open_window(parts[1].clone(), parts[2].clone(), parts[3].clone(), parts[4].clone())
                    );

                }
                if line.contains("closewindow") && !ignored_handler.should_ignore(&parts[0]) {
